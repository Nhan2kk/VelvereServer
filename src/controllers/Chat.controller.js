...
exports.assignAdminToSession = exports.getUserChatSessions = exports.getAllChatSessions = exports.closeChatSession = exports.markMessagesAsRead = exports.getMessagesBySessionId = exports.sendMessage = exports.createChatSession = void 0;
const ChatMessage_model_1 = __importDefault(require("../models/ChatMessage.model"));
const ChatSession_model_1 = __importDefault(require("../models/ChatSession.model"));
...
const createChatSession = async (req, res) => {
    try {
        ...
        // Tạo phiên chat mới
        const newSession = new ChatSession_model_1.default({
        ...
        const welcomeMessage = new ChatMessage_model_1.default({
...
const sendMessage = async (req, res) => {
    try {
        ...
        const session = await ChatSession_model_1.default.findById(session_id);
        if (!session) {
...
const getMessagesBySessionId = async (req, res) => {
    try {
        ...
        const messages = await ChatMessage_model_1.default.find({ session_id: sessionId })
            .sort({ createdAt: 1 });
...
const markMessagesAsRead = async (req, res) => {
    try {
        ...
        await ChatMessage_model_1.default.updateMany({ session_id: sessionId, sender_type: oppositeType, read: false }, { read: true });
...
const closeChatSession = async (req, res) => {
    try {
        const { sessionId } = req.params;
        const session = await ChatSession_model_1.default.findById(sessionId);
        if (!session) {
            res.status(404).json({ message: 'Không tìm thấy phiên chat' });
            return;
        }
...
const getAllChatSessions = async (req, res) => {
    try {
        ...
        const sessions = await ChatSession_model_1.default.find(query)
            .sort({ last_message: -1 });
...
const getUserChatSessions = async (req, res) => {
    try {
        ...
        const sessions = await ChatSession_model_1.default.find({ user_id: userId })
            .sort({ last_message: -1 });
        res.status(200).json(sessions);
    }
    catch (err) {
...
const assignAdminToSession = async (req, res) => {
    try {
        const { sessionId } = req.params;
...
exports.assignAdminToSession = assignAdminToSession;
